nombre = "I am Iron Man" #esto le da el tipo de dato que
                         #que es la variable
print(nombre) #imprime lo que contiene nombre
print(type(nombre))  # imprime el tipo de dato que es la variable


#En este caso la variable nombre, su tipo es definido por
# el contenido que a esta se le da, en el caso anterior se
# le entrego un contenido de tipo string, y ahora uno int
# y en la función type() se puede observar que cambio de ser 
# str a int#
nombre = 5
print(nombre)
print(type(nombre))

#Al tener que crear una variable que contenga  más de una palabra, estas
# se deben de separar con un guión bajo como se muestra a continuación#
nombre_alumno = "Osiris Mendoza"
